...
Function bt_StatusBarHandle (Win)
   ...
   FOR k := 1 TO oHmgApp():ControlCount
       IF ControlByIndex( k ):Type == "STATUSBAR" .AND. ControlByIndex( K ):ParentFormHandle == hWnd
          hWndStatusBar := ControlByIndex( k ):Handle
       ENDIF
   NEXT
...
Function bt_ToolBarBottomHandle (Win)
   ...
   FOR k := 1 TO oHmgApp():ControlCount
       IF ControlByIndex( k ):Type == "TOOLBAR" .AND. ControlByIndex( K ):ParentFormHandle == hWnd .AND. ControlByIndex( K ):CTRL018 == .T. .AND. ControlByIndex( K ):CTRL019 == .F.
          hWndToolBar := ControlByIndex( k ):Handle
       ENDIF
   NEXT
Return hWndToolBar
...
Function bt_ToolBarTopHandle (Win)
   ...
   LOCAL k, hWndToolBar := 0
   FOR k := 1 TO oHmgApp():COntrolCount
       IF ControlByIndex( k ):Type == "TOOLBAR" .AND. ControlByIndex( K ):ParentFormHandle == hWnd .AND. ControlByIndex( K ):CTRL018 == .F. .AND. ControlByIndex( K ):CTRL019 == .F.
          hWndToolBar := ControlByIndex( k ):Handle
       ENDIF
   NEXT
Return hWndToolBar
...
LOCAL k, hBitmap := 0
   ...
   IF k > 0 .AND. GetControlType (cControlName, cFormName) == "IMAGE"
      ...
      #else             // HMG Official
         hBitmap := ControlByIndex( K ):CTRL037
      #endif
   ENDIF
Return hBitmap
...
LOCAL hWnd, k
   ...
   IF k > 0 .AND. GetControlType (cControlName, cFormName) == "IMAGE"
      ...
      #else             // HMG Official
         IF ControlByIndex( K ):CTRL037 <> 0 .AND. lReleasePreviousBitmap == .T.
            BT_BitmapRelease ( ControlByIndex( K ):CTRL037 )
         ENDIF
         ControlByIndex( K ):CTRL037 := hBitmap
         ControlByIndex( K ):CTRL020 := BT_BitmapWidth  (hBitmap)
         ControlByIndex( K ):CTRL021 := BT_BitmapHeight (hBitmap)
      #endif
