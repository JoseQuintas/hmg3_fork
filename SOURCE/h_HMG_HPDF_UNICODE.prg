...
function _HMG_HPDF_MULTILINE_PRINT_UNICODE ( nRow, nCol, nToRow, nToCol, cFontName, nFontSize, aFontColor, cText, lBold, lItalic, lUnderline, lStrikeout, lColor, lFont, lSize, cAlign )
   ...
   default cFontName := oHmgApp():APP150[ 8 ]
   default nFontSize := oHmgApp():APP150[ 9 ]
   ...
   if oHmgApp():APP150 [ 1 ] == nil // PDF object not found!
      _HMG_HPDF_Error( 3 )
      return nil
   endif
   if oHmgApp():APP150 [ 7 ] == nil // PDF Page object not found!
      _HMG_HPDF_Error( 5 )
      return nil
   endif
...
function _HMG_HPDF_DRAWIMAGERAW ( cRawImage, nRow, nCol, nToRow, nToCol, nImageWidth, nImageHeight )
   local nWidth := oHmgApp():APP150 [ 4 ]
   local nHeight := oHmgApp():APP150 [ 5 ]
   ...
   oImage := _HPDF_LOADRAWBITMAP ( oHmgApp():APP150 [ 1 ], cRawImage, nImageWidth, nImageHeight)
   ...
   HPDF_Page_DrawImage( oHmgApp():APP150 [ 7 ], oImage, nxPos, nyPos - _HMG_HPDF_MM2Pixel( nToRow - nRow ), _HMG_HPDF_MM2Pixel( nToCol - nCol ), _HMG_HPDF_MM2Pixel( nToRow - nRow ) )
return nil
